...
| **EF Core primitive collections** | `Recipe.Instructions` → `text[]` | EF 9+ maps `IReadOnlyList<string>` to a native PostgreSQL array with no configuration. Cheap, ordered, and queryable — but the element has no shape, which is why ingredients moved off it (ADR-022). |
