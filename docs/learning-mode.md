...
| **Entity equality by identity** | `RecipeManager.Domain/Shared/Entity.cs` | Two `Recipe` objects are equal if their ids match, regardless of field values — the opposite of a value object. Note `Equals` compares type **and** id while `GetHashCode` hashes id **alone**: the contract only requires equal objects to hash equally, never the reverse, so unequal objects are always allowed to collide. |
| **EF Core primitive collections** | `InstructionStep.IngredientIds` → `uuid[]`, mapped with `PrimitiveCollection` in `RecipeConfiguration.cs` | A native PostgreSQL array, ordered and queryable, with no join table. Right for a list of plain values; wrong once each element needs its own fields — which is why ingredients and steps moved off `text[]` (ADR-022, `R-17`). Note the cost: an array element cannot carry a foreign key. |
| **Anti-corruption at the boundary** | `RecipeManager.Application/Mappings/InstructionMappingExtensions.cs` | The request references ingredients by *index into the same payload*; the mapper translates that into the *ids* the domain speaks before `Recipe` sees anything (ADR-023). The aggregate never learns what a payload looks like. |
| **Owned entity collection (`OwnsMany`)** | `Recipe.Ingredients` → the `"RecipeIngredients"` child table, mapped in `RecipeManager.Infrastructure/Context/Configurations/RecipeConfiguration.cs` | A child table with structure and a real FK, still saved in one call because `Recipe` owns it. Compare with a second aggregate, which would force the unit-of-work decision ADR-006 deferred. The costs it brings: an explicit `Position` column, because rows have no inherent order, and `ValueGeneratedNever()` on the key, because the domain mints its own ids. |
